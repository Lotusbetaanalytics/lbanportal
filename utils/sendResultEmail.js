...
const updateResultEmail = async (req, existingResult, result, hrEmail) => {
  ...
  if (existingResult.score != req.body.score) {
    ...
    try {
      ...
      content = `
      Kindly be aware that ${userDetails.fullname}  has updated the self-appraisal section of the ${convertQuarter(existingResult.quarter)} performance appraisal with an overall score of ${result.score}., 
      Kindly log on to the <a href="https://lbanstaffportal.herokuapp.com/dashboard">Portal</a> for your final rating.
      `
      await sendEmail({
  ...
  if (existingResult.managerscore != req.body.managerscore) {
    ...
    try {
      ...
      salutation = `Dear Manager,`
      content = `
      Kindly be aware that the updated manager's rating score for ${userDetails.fullname} for the ${convertQuarter(existingResult.quarter)} performance appraisal is ${result.managerscore}, 
...
const rejectedResultEmail = async (req, existingResult, result, hrEmail) => {
  ...
  try {
    let salutation = ``
    let content = `
    You have rejected your manager's score for the ${convertQuarter(existingResult.quarter)} performance appraisal. 
    Your self appraisal score is ${result.score} and your manager's score is ${result.managerscore}.
  ...
  try {
    ...
    let salutation = `Dear Manager,`
    let content = `
    Kindly be aware that ${userDetails.fullname}  has rejected the manager score section of the ${convertQuarter(existingResult.quarter)} performance appraisal. 
...
const acceptedResultEmail = async (req, existingResult, result, hrEmail) => {
  ...
  try {
    ...
    let salutation = ``
    let content = `
    Kindly be aware that ${userDetails.fullname} has accepted the manager score of the ${convertQuarter(existingResult.quarter)} performance appraisal. 
